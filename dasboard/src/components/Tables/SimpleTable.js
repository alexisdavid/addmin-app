...
const TableHeader = ({ columns,data }) => {
    return (
        <>
        <thead>
            <tr>
                {columns.map(column => <th className={column.customClass}>{column.label}</th>  )}
            </tr>
        </thead>
        <tbody>
            {data.map((linea,key) =>(
                <tr key={key}>
                 {columns.map((column,c) => <td key={c}>{column.customRender?column.customRender(linea[column.body]):linea[column.body]}</td>)}
                </tr>
            ))}
        {/* <tr>
            <td>
